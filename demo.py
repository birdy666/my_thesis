...
if __name__ == "__main__":
    checkpoint = torch.load('./models/checkpoints/epoch_1379' + ".chkpt", map_location=torch.device('cpu')) #in docker
    #checkpoint = torch.load('/media/remote_home/chang/z_master-thesis/models/checkpoints/epoch_79' + ".chkpt")
    ...
    for i in range(len(eft_all_fake)): 
        ...
        if len(caption_without_punctuation.split()) < cfg.MAX_SENTENCE_LEN:
            #vector, mask  = get_caption_vector(text_model, caption_without_punctuation, cfg.MAX_SENTENCE_LEN, cfg.D_WORD_VEC)
            data['vector'] = vector_mask[i]['vector']
            data['vec_mask'] = vector_mask[i]['vec_mask']
            
            text_match = torch.tensor(data['vector'], dtype=torch.float32).unsqueeze(0).to(device)
            text_match_mask = torch.tensor(data['vec_mask'], dtype=torch.int).unsqueeze(0).to(device)
            noise = torch.randn((1, 24, 150), dtype=torch.float32).to(device)
        
            so3_fake = net_g(text_match, text_match_mask, noise)            
            so3_fake = so3_fake[0].detach().cpu().numpy()
    ...
    with open('./demo/vector.json', 'w') as f:
        json.dump(output, f)
