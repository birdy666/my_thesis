...
class Generator(nn.Module):
    def __init__(self, cfg):
        ...
        self.fc_stack = nn.ModuleList([nn.Linear(cfg.D_MODEL_LIST_G[i], cfg.D_MODEL_LIST_G[i+1], bias=False) for i in range(cfg.N_LAYERS_G)])
    def forward(self, noise, input_vec, src_mask, return_attns=False):
        ...
        for i in range(self.cfg.N_LAYERS_G):
            """
