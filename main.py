...
if __name__ == "__main__":
    ...
    optimizer_g = ScheduledOptim(
        ...
        lr_mul=2.0, d_model=300, n_warmup_steps=1300, n_steps=checkpoint['n_steps_g'] if checkpoint!=None else 0)
    optimizer_d = ScheduledOptim(
        torch.optim.Adam(net_d.parameters(), betas=(cfg.BETA_1, cfg.BETA_2), eps=1e-09),
        lr_mul=2.0, d_model=300, n_warmup_steps=3000, n_steps=checkpoint['n_steps_d'] if checkpoint!=None else 0)    
