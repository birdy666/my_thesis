...
if __name__ == "__main__":
    ...
    net_g, net_d_1, net_d_2 = getModels(cfg, device, checkpoint) 
    ...
    optimizer_g = ScheduledOptim(
        torch.optim.Adam(net_g.parameters(), lr=cfg.LEARNING_RATE_G, betas=(cfg.BETA_1, cfg.BETA_2), eps=1e-09, weight_decay=0.001),
        lr_mul=2.0, d_model=300, n_warmup_steps=cfg.N_WARMUP_STEPS_G, n_steps=checkpoint['n_steps_g'] if checkpoint!=None else 0)
    optimizer_d_1 = ScheduledOptim(
        torch.optim.Adam(net_d_1.parameters(), lr=cfg.LEARNING_RATE_D_1, betas=(cfg.BETA_1, cfg.BETA_2), eps=1e-09, weight_decay=0.001),
        lr_mul=2.0, d_model=300, n_warmup_steps=cfg.N_WARMUP_STEPS_D, n_steps=checkpoint['n_steps_d'] if checkpoint!=None else 0)    
    optimizer_d_2 = ScheduledOptim(
        torch.optim.Adam(net_d_2.parameters(), lr=cfg.LEARNING_RATE_D_2, betas=(cfg.BETA_1, cfg.BETA_2), eps=1e-09, weight_decay=0.001),
        lr_mul=2.0, d_model=300, n_warmup_steps=cfg.N_WARMUP_STEPS_D, n_steps=checkpoint['n_steps_d'] if checkpoint!=None else 0)    
    
    criterion = nn.BCELoss()

    train(cfg, device, net_g, net_d_1, net_d_2, optimizer_g, optimizer_d_1, optimizer_d_2, criterion, dataLoader_train, dataLoader_val)
